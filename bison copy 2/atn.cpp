...
// TODO delete
// void Atn::addMainElement(wstring ws, freeling::tree<ASTN*>* t)
// {
//     m_func[ws] = t;
// }
...
vector<Atn::Output> Atn::executeAtn(wstring atnname, const vector<wstring>& in, int init, int act) {
...
vector<Atn::Output> Atn::executeState(wstring atnname, const vector<wstring>& in, const freeling::tree<ASTN*>& state, int i, int j, map<wstring, Data*> global, const vector<wstring>& finals, const map<wstring, freeling::tree<ASTN*>*>& states, bool final) {
    // Inicializate de stack of the state & output
    /*stack< map<wstring, Data*> > m_stack;*/ map<wstring, Data*> m;
    m_stack.push(m);
    ...
    if (final) {
        if (m_stack.top().find(L"@") != m_stack.top().end()) {
            ...
            if (value->isWstring()) {
                ...
                vector<Atn::Output> output = executeAtn(atnname, in, j, j);
    ...
    for (int k = 0; k < list.num_children(); ++k) {
        ...
        wstring nextState = (*((list.nth_child(k)).nth_child(0)))->getValueWstring();
        ...
        auto it = (list.nth_child(k)).nth_child(1);
        ASTN* node = *(it.begin());
        wstring token = node->getToken();
        if (token == L"TOKEN ID" && m_atn.find(node->getValueWstring()) != m_atn.end()) { // expr is an atn
            vector<Atn::Output> output = executeAtn(node->getValueWstring(), in, i, j);
            actualOutput.insert( actualOutput.end(), output.begin(), output.end() );
            for (int l = 0; l < output.size(); ++l) {
                vector<Atn::Output> newOutput = executeState(atnname, in, *(st->second), output[l].init, output[l].final, global, finals, states, stateFinal > -1);
                actualOutput.insert( actualOutput.end(), newOutput.begin(), newOutput.end() );
            }
        }
        else { // expr is not an atn
            Data* value = evaluateExpression(it, global,/* m_stack,*/ in, j);
            checkBool(value);
            if (value->getBoolValue()) {
                vector<Atn::Output> output = executeState(atnname, in, *(st->second), i, j + 1, global, finals, states, stateFinal > -1);
                actualOutput.insert( actualOutput.end(), output.begin(), output.end() );
            }
        }
    }
...
Data* Atn::executeFunction(wstring funcname, const freeling::tree<ASTN*>::const_iterator& args, map<wstring, Data*>& global, /*stack< map<wstring, Data*> > m_stack,*/ const vector<wstring>& in, int input) {
...
map<wstring, Data*> Atn::listArguments(const freeling::tree<ASTN*>::const_iterator& AstF, const freeling::tree<ASTN*>::const_iterator& args, map<wstring, Data*>& global, /*stack< map<wstring, Data*> > m_stack,*/ const vector<wstring>& in, int input) {
...
Data* Atn::executeListInstructions(const freeling::tree<ASTN*>::const_iterator& t, map<wstring, Data*>& global, /*stack< map<wstring, Data*> > m_stack,*/ const vector<wstring>& in, bool final) {
    for (int i = 0; i < t.num_children(); ++i) {
...
Data* Atn::executeInstruction(const freeling::tree<ASTN*>::const_iterator& it, map<wstring, Data*>& global, /*stack< map<wstring, Data*> > m_stack,*/ const vector<wstring>& in, bool final) {
    ASTN node = *(*it);
...
Data* Atn::evaluateExpression(const freeling::tree<ASTN*>::const_iterator& it, map<wstring, Data*>& global, /*stack< map<wstring, Data*> > m_stack,*/ const vector<wstring>& in, int input) {
    Data *value1, *value2;
    ASTN node = *(*it);
    wstring type = node.getToken();
...
Data* Atn::getAccesData(const ASTN& t, const freeling::tree<ASTN*>::const_iterator& it, map<wstring, Data*>& global, /*stack< map<wstring, Data*> > m_stack,*/ const vector<wstring>& in, int input) {
    Data* v = nullptr;
...
Data* Atn::evaluateBool(wstring type, Data* v, const freeling::tree<ASTN*>::const_iterator& t, map<wstring, Data*>& global, /*stack< map<wstring, Data*> > m_stack,*/ const vector<wstring>& in, int input) {
    if (type == L"AND") {
